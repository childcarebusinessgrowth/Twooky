...
export async function POST(request: Request) {
  try {
    ...
    const { data: ownedProfile, error: ownedProfileError } = await admin
      .from("provider_profiles")
      .select("profile_id, listing_status")
      .eq("owner_profile_id", user.id)
      .limit(1)
      .maybeSingle()

    if (ownedProfileError) {
      return NextResponse.json({ success: false, error: ownedProfileError.message }, { status: 400 })
    }

    let providerProfileId = ownedProfile?.profile_id ?? null
    let existingListingStatus = ownedProfile?.listing_status ?? null

    if (!providerProfileId) {
      const { data: selfOwnedProfile, error: selfOwnedProfileError } = await admin
        .from("provider_profiles")
        .select("profile_id, listing_status")
        .eq("profile_id", user.id)
        .maybeSingle()

      if (selfOwnedProfileError) {
        return NextResponse.json({ success: false, error: selfOwnedProfileError.message }, { status: 400 })
      }

      providerProfileId = selfOwnedProfile?.profile_id ?? user.id
      existingListingStatus = selfOwnedProfile?.listing_status ?? null
    }
