...
import {
  ...
  deleteProviderBlogPost,
  publishProviderBlogPost,
  unpublishProviderBlogPost,
  updateProviderBlogPost,
  uploadProviderBlogImage,
} from "./actions"

export type ProviderBlogRecord = {
  id: string
  slug: string
  title: string
  excerpt: string
...
function toForm(blog: ProviderBlogRecord): BlogFormState {
  return {
    title: blog.title,
    slug: blog.slug,
...
type Props = {
  initialPosts: ProviderBlogRecord[]
  hasWebsite: boolean
  websiteId: string | null
}

export function ProviderBlogsPageClient({ initialPosts, hasWebsite, websiteId }: Props) {
  const router = useRouter()
  const { toast } = useToast()
  const [isPending, startTransition] = useTransition()
  const [activeTab, setActiveTab] = useState<"manage" | "editor">("manage")
  const [search, setSearch] = useState("")
  const [selectedBlogId, setSelectedBlogId] = useState<string | null>(initialPosts[0]?.id ?? null)
  const [manageForm, setManageForm] = useState<BlogFormState>(initialPosts[0] ? toForm(initialPosts[0]) : EMPTY_FORM)
  ...
  const blogs = useMemo(() => initialPosts, [initialPosts])
  ...
  const uploadCover = async (file: File, target: "manage" | "editor") => {
    if (!websiteId) {
      setError("Create your site first to upload images.")
      return
    }
    setError(null)
    setNotice(null)
    ...
    startTransition(async () => {
      try {
        const uploaded = await uploadProviderBlogImage(websiteId, file)
        if (target === "manage") {
          setManageForm((prev) => ({
            ...prev,
  ...
  const uploadInlineImage = async (file: File): Promise<string> => {
    if (!websiteId) {
      throw new Error("Create your site first to upload images.")
    }
    const uploaded = await uploadProviderBlogImage(websiteId, file)
  ...
  const handleSaveManage = () => {
    ...
    runAction(
      async () => {
        await updateProviderBlogPost(selectedBlog.id, toInput(manageForm))
      },
      "Post updated.",
    )
  }
  ...
  const handleDeleteBlogConfirm = async () => {
    ...
    setError(null)
    try {
      await deleteProviderBlogPost(selectedBlog.id)
  ...
  const handleTogglePublish = () => {
    ...
    runAction(
      async () => {
        if (shouldPublish) {
          await publishProviderBlogPost(selectedBlog.id)
        } else {
          await unpublishProviderBlogPost(selectedBlog.id)
        }
      },
      shouldPublish ? "Post published." : "Post moved to draft.",
    )
  }
  ...
  const handleCreate = (status: "draft" | "published") => {
    ...
    runAction(
      async () => {
        await createProviderBlogPost(toInput(payload))
  ...
  if (!hasWebsite) {
    return (
      <div className="mx-auto max-w-lg space-y-6 rounded-xl border border-dashed border-border bg-card p-8 text-center">
        <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-xl bg-primary/10">
          <LayoutTemplate className="h-6 w-6 text-primary" />
        </div>
        <div className="space-y-2">
          <h1 className="text-xl font-semibold text-foreground">Create your mini-site first</h1>
          <p className="text-muted-foreground text-sm leading-relaxed">
            Choose a template and publish your site, then you can add blog posts that appear on your public site.
          </p>
        </div>
        <Button asChild>
          <Link href="/dashboard/provider/website">Browse templates</Link>
        </Button>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground max-w-2xl">
          Posts appear on your published mini-site when you publish them here.
        </p>
      </div>
